...
if __name__ == "__main__":
    ...
    for nodeID, entries in entries_by_node.items():
        ...
        gaps_report_file_path = f'{base_file_name}_gaps_report_{nodeID}.txt'
        with open(gaps_report_file_path, 'w') as report_file:
            ...
            # Final gap calculation moved outside the if condition to ensure it's always considered
            if merged_intervals:
        ...
        # Append the total gap duration to a different file
        gap_duration_analysis_file_path = f'{base_file_name}_gap_duration_analysis_{nodeID}.txt'
        with open(gap_duration_analysis_file_path, 'a') as analysis_file:
            analysis_file.write(total_gap_info)

        # Print the total gap info to console for confirmation
        print(f'Node {nodeID}: {total_gap_info}', end='')
