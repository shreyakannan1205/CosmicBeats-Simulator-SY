...
# Extract just the number part of the satellite for xticks
satellite_numbers = [int(sat.split("_")[1]) for sat in weighted_avg_coverage_time_diff.keys()]

# Plotting the population-weighted average coverage time difference for each satellite
plt.figure(figsize=(15, 6))
plt.plot(satellite_numbers, list(weighted_avg_coverage_time_diff.values()), marker='o', linestyle='-', color='skyblue')
plt.xlabel('Angle Difference from Satellite 1')
plt.ylabel('Population-Weighted \n Average Coverage Time Difference \n (seconds)')
# plt.title('Population-Weighted Average Coverage Time Difference Across Different Satellites')
plt.grid(True)
# plt.show()
plt.savefig("plot_gap_duration_difference_plus1_90.png")
