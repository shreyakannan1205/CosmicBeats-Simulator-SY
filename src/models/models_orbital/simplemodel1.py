...
class simplemodel1(IModel):
    ...
    __dependencies = [['ModelImagingRadio']]
    ...
    def Execute(self):
        ...
        """
        #print(self.__ownernode.timestamp.to_unix(), self.__ownernode.nodeID)
        ...
        _channels = self.__ownernode.has_ModelWithTag(EModelTag.IMAGINGRADIO).call_APIs("update_Channel")
        if _channels is not None and len(_channels) > 0:
            for _channel in _channels:
                _devices = _channel.get_Devices()
                assert len(_devices) == 2
                _device0 = _devices[0]
                _device1 = _devices[1] #one of these devices is the owner node - dk which one

                _device0Pos = _device0.get_OwnerNode().get_Position(_device0.get_OwnerNode().timestamp)
                _device1Pos = _device1.get_OwnerNode().get_Position(_device1.get_OwnerNode().timestamp)
                _link = ImagingLink(_device0, _device1, _device0Pos.get_distance(_device1Pos))
                print(_link.get_SNR(), _device0.get_OwnerNode().nodeID, _device1.get_OwnerNode().nodeID, self.__ownernode.timestamp, _device0Pos.get_distance(_device1Pos))
            #print(_channels, self.__ownernode.timestamp)

def init_simplemodel1(
        _ownernodeins: INode, 
        _loggerins: ILogger, 
        _modelArgs) -> IModel:
    '''
    ...
    return simplemodel1(_ownernodeins, _loggerins, _alwaysCalc)